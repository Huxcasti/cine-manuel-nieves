  ...
  const pendingTickets = await pool.query(
    `
      ...
      WHERE payment_status = 'pending'
        ...
        AND COALESCE(payment_hold_until,
          created_at + ($1 * INTERVAL '1 minute')) <= NOW();
    `,
    [PENDING_RESERVATION_MINUTES]
  );

  // Conserva las referencias de PayPal para poder reconciliar avisos tardíos.
  // El asiento ya queda libre cuando vence payment_hold_until.
  await pool.query(`
    UPDATE tickets SET payment_status = 'expired', payment_hold_until = NULL
    WHERE payment_status = 'pending' AND paypal_order_id IS NOT NULL
      AND COALESCE(payment_hold_until, created_at) <= NOW() - INTERVAL '24 hours';
  `);
...
app.get("/api/seats", async (req, res) => {
  try {
    const showtimeId = normalizeShowtimeId(req.query.showtimeId);
    ...
    const ticketResult = await pool.query(
      `
        ...
        FROM tickets
        WHERE
          LOWER(TRANSLATE(BTRIM(customer->>'showtimeId'), '-{}', '')) = REPLACE($1::text, '-', '')
          AND (
            payment_status IN (
              'paid',
              'approved'
...
function normalizeShowtimeId(value) {
  const hex = String(value || "").trim().replace(/[{}-]/g, "").toLowerCase();
  if (!/^[0-9a-f]{32}$/.test(hex)) return "";
  return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20)}`;
}

// Siempre se bloquea la tanda ANTES del boleto, igual que al reservar.
// Así una reserva nueva espera incluso si el plazo vence durante un cobro.
async function lockPaymentReservation(client, reservationId) {
  const initial = await client.query(
    `SELECT customer->>'showtimeId' AS showtime_id FROM tickets WHERE id = $1;`,
    [reservationId]
  );
  if (!initial.rowCount) return { ticket: null, showtime: null };

  const showtimeId = normalizeShowtimeId(initial.rows[0].showtime_id);
  const showtimeResult = showtimeId
    ? await client.query(
        `SELECT id, show_date, show_time, active FROM showtimes WHERE id = $1 FOR UPDATE;`,
        [showtimeId]
      )
    : { rows: [] };
  ...
  return { ticket: result.rows[0] || null, showtime: showtimeResult.rows[0] || null };
}

// Solo se invoca dentro de una transacción que mantiene bloqueada la tanda.
async function checkPaymentSeats(client, ticket, showtime) {
  if (!showtime || !showtime.active ||
      normalizeShowtimeId(ticket.customer?.showtimeId) !== showtime.id) {
    return { reason: "La tanda ya no está disponible.", seats: [] };
  }
  const currentTime = `${normalizeTicketShowDate(showtime.show_date)} ${normalizeTicketShowTime(showtime.show_time)}`;
  if (currentTime !== ticket.show_time) {
    return { reason: "La fecha u hora de la tanda cambió después de reservar.", seats: [] };
  }
  if (!Array.isArray(ticket.seats) || !ticket.seats.length ||
      ticket.seats.some(seat => !OFFICIAL_SEAT_IDS.has(seat))) {
    return { reason: "Los asientos de la reservación no son válidos.", seats: [] };
  }

  const conflicts = await client.query(
    `SELECT seats FROM tickets
     WHERE LOWER(TRANSLATE(BTRIM(customer->>'showtimeId'), '-{}', '')) = REPLACE($1::text, '-', '')
       AND id <> $2 AND seats && $3::text[]
       AND (payment_status IN ('paid', 'approved') OR
         (payment_status = 'pending' AND COALESCE(payment_hold_until,
           created_at + ($4 * INTERVAL '1 minute')) > NOW()));`,
    [showtime.id, ticket.id, ticket.seats, PENDING_RESERVATION_MINUTES]
  );
  const occupied = new Set(conflicts.rows.flatMap(row => row.seats || []));
  const seats = ticket.seats.filter(seat => occupied.has(seat));
  return {
    reason: seats.length ? "Uno o más asientos ya pertenecen a otra reservación." : "",
    seats
  };
}

function paymentReviewResponse(ticket) {
  return {
    code: "PAYMENT_REVIEW_REQUIRED",
    reservationId: ticket.id,
    error: "PayPal confirmó tu pago, pero tu entrada necesita revisión del cine. " +
      "No vuelvas a pagar. Comunícate con el cine e indica la referencia " + ticket.id + "."
  };
}
...
app.post("/api/reservations", reservationCreateLimiter, async (req, res) => {
  ...
  customer
    ...
    const showtimeId = normalizeShowtimeId(rawShowtimeId);
...
Este endpoint libera reservas pendientes. Si ya tienen una orden de PayPal,
conserva el registro para reconciliar notificaciones posteriores.
*/
...
app.delete("/api/reservations/:id/cancel", async (req, res) => {
  ...
  try {
    ...
    await client.query("BEGIN");
    const { ticket } = await lockPaymentReservation(client, reservationId);
    ...
    if (["cancelled", "expired"].includes(ticket.payment_status)) {
      await client.query("COMMIT");
      return res.json({ success: true, removed: false, released: true });
    }
    ...
    if (ticket.paypal_order_id) {
      await client.query(
        `UPDATE tickets SET payment_status = 'cancelled', payment_hold_until = NULL
          WHERE id = $1;`,
        [reservationId]
      );
    } else {
      await client.query(`DELETE FROM tickets WHERE id = $1;`, [reservationId]);
    }
    await client.query("COMMIT");
    return res.json({ success: true, removed: !ticket.paypal_order_id, released: true });
  } catch (error) {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { ticket, showtime } = await lockPaymentReservation(client, reservationId);
      ...
      const seatCheck = await checkPaymentSeats(client, ticket, showtime);
      if (seatCheck.reason) {
        await client.query("ROLLBACK");
        return res.status(409).json({ error: seatCheck.reason + " Selecciona otra tanda o asiento." });
      }
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (ticket.payment_status !== "pending") {
        ...
        if (ticket.payment_status === "payment_review" && ticket.paypal_order_id === orderId) {
          return res.status(409).json(paymentReviewResponse(ticket));
        }
      ...
      if (seatCheck.reason) {
        await client.query("ROLLBACK");
        return res.status(409).json({
          error: seatCheck.reason + " No se realizó un nuevo cobro."
        });
      }

      // Se mantienen bloqueadas tanda y reserva hasta confirmar el resultado.
      // La creación de reservas usa el mismo bloqueo de tanda.
...
async function recordTicketCheckin(client, ticket, actor) {
  await client.query(
    `INSERT INTO checkins (
      id, ticket_id, employee_id, employee_name, employee_username,
      seats_count, movie, show_time, seats
    ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9);`,
    [crypto.randomUUID(), ticket.id, actor.id || null, actor.name,
      actor.username, ticket.seats.length, ticket.movie, ticket.show_time, ticket.seats]
  );
}
...
app.post("/api/employee/checkin", requireEmployee, async (req, res) => {
  ...
  try {
    ...
    if (!["paid", "approved"].includes(ticket.payment_status)) {
      ...
      return res.status(400).json({
        error: ticket.payment_status === "payment_review"
          ? "Este pago necesita revisión del administrador. La entrada todavía no es válida."
    ...
    */
    ...
    await recordTicketCheckin(client, ticket, req.employee);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: ticket.payment_status === "payment_review"
            ? "Este pago necesita revisión. La entrada todavía no es válida."
            : "El boleto no ha sido pagado."
        });
      }
      ...
      const dateValidation = getTicketDateValidation(ticket);
      if (!dateValidation.allowed) {
        await client.query("ROLLBACK");
        return res.status(dateValidation.status).json({ error: dateValidation.error });
      }
      if (!Array.isArray(ticket.seats) || !ticket.seats.length) {
        await client.query("ROLLBACK");
        return res.status(400).json({ error: "El boleto no contiene asientos válidos." });
      }
      ...
      await recordTicketCheckin(client, ticket, {
        id: null, name: "Administrador", username: "administrador"
      });
      await client.query("COMMIT");
  ...
  try {
    ...
    if (eventType === "PAYMENT.CAPTURE.COMPLETED") {
      ...
      try {
        ...
        // Un evento repetido nunca reactiva un boleto reembolsado/revertido
        // ni convierte automáticamente una incidencia en una entrada válida.
        if (["payment_review", "refunded", "reversed"].includes(ticket.payment_status)) {
          await client.query("COMMIT");
          return res.json({ success: true, alreadyProcessed: true,
            manualReview: ticket.payment_status === "payment_review" });
        }
        ...
        if (["paid", "approved"].includes(ticket.payment_status)) {
          ...
          if (!ticket.ticket_email_sent_at) {
            try {
              ...
              console.error("No se pudo recuperar el envío del boleto:", emailError);
            }
          }
          return res.json({ success: true, alreadyProcessed: true });
        }
        ...
        const seatCheck = await checkPaymentSeats(client, ticket, showtime);
        const reviewReason = seatCheck.reason ||
          (ticket.payment_status !== "pending"
            ? "El pago llegó después de cancelar o cerrar la reservación."
            : "");

        if (reviewReason) {
          updatedCustomer.paymentReviewReason = reviewReason;
          updatedCustomer.paymentReviewSeats = seatCheck.seats;
          updatedCustomer.paymentReviewAt = new Date().toISOString();
          await client.query(
            `UPDATE tickets SET payment_status = 'payment_review', customer = $2,
               cancellation_token_hash = NULL, payment_hold_until = NULL
             WHERE id = $1;`,
            [reservationId, JSON.stringify(updatedCustomer)]
          );
          await client.query("COMMIT");
          console.warn("Pago recibido para revisión del cine:", { reservationId, reason: reviewReason });
          return res.json({ success: true, manualReview: true, reservationId });
        }
